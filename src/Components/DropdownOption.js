      ...
      {({ open }) => (
        <>
          <div className="mt-1 relative">
            <Listbox.Button
              className={`relative w-full bg-[#F3E8FF] h-[3rem] overflow-hidden flex items-center border rounded-md pl-3 pr-10 py-2 text-left cursor-pointer focus:outline-[#772EDA] focus:ring-1 focus:ring-[#772EDA] focus:border-[#772EDA] sm:text-sm ${
                selected.id == 0 && "pl-2"
              }`}
            >
              <span
                className={`flex items-center gap-3 pl-2 ${
                  selected.id == 0 && "gap-0 pl-[3px]"
                }`}
              >
                <img
                  src={selected.image}
                  alt=""
                  className={`rounded-full ${selected.id == 0 && "w-[2.7rem]"}`}
                />
